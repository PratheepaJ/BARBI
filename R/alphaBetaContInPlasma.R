...
alphaBetaContInPlasma <-  function(psPlByBlock,
                                   psallzeroInNC,
                                   blk,
                                   alphaBetaNegControl,
                                   stringent = FALSE){
        ...
        gammaPrior_c <- lapply(samples_in_block, function(x){
                ...
                if(stringent==TRUE){
                        ...
                        beta_ij_c[ind_mean_less_1] <- 1/(gamma_ij_0[ind_mean_less_1 ]*mu_ij_0[ind_mean_less_1])
                ...
                all_zero_neg_controls <- ifelse(species_name %in% taxa_names(psallzeroInNC[[blk]]), "Yes", "No")

                rt <- list(alpha_ij_c,
                           beta_ij_c,
                           kij,
                           species_name,
                           all_zero_neg_controls,
                           S_j,
                           S_j0)
                names(rt) = c("alpha_ij_c",
                              "beta_ij_c",
                              "kij",
                              "species_name",
                              "all_zero_neg_controls",
