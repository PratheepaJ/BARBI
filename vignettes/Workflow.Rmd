...
blocks <-ifelse(ext.num %in% set1, "Set1", ifelse(ext.num %in% set2, "Set2", "Set3"))
...
ps <- subset_samples(ps,SampleType %in% c("Plasma","Control"))
```
...
taxa.post.all.sam  <- samplingPosterior(psPlByBlock=psPlByBlock,blk=3,gammPrior=gammPrior,iter=10000)
```
