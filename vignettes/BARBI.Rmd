...
for(blk in 1:num_blks){
        ...
        total_summary_table <- NULL #Summary dataframe for all taxa from all subjects in a block.
        ...
        for(sam in 1:nsamples(psPlByBlock[[blk]])){
          ...
          all.zero.nc <- list()
                  ...
                  all_real_taxa <- c(all_real_taxa, as.character(df$Species))
                ...
                df.p <- gtable_add_rows(df.p,heights = grobHeight(title) + padding,pos = 0)
...
total_summary_table_list  <- lapply(final_results_list, function(x) {x[[1]]} )
total_summary_table_all <- do.call("rbind", total_summary_table_list )
row.names(total_summary_table_all) <- total_summary_table_all[,1]
colnames(total_summary_table_all) <- c("Sample", "Species (Raw Reads, Lower Bound Background Reads Estimate, Upper Bound Real Reads Estimate, Presence in Neg. Controls)")
total_summary_table_all <- total_summary_table_all[order(row.names(total_summary_table_all)), , drop = FALSE]

write.table(total_summary_table_all, paste("./total.summary.table.95", ".txt", sep=""), sep="\t", col.names=TRUE, row.names = FALSE)

saveRDS(finalPosTaxa,file="./finalPosTaxa.rds")
```
