...
The suggested number of itera is 10,000.
...
The contamination identification is based on the lower limit of the true signal and the upper limit of the contaminant. 
...
all_taxa_res <- makeTable(itera, burnIn, num_blks, cov.pro, psByBlock, gammaPrior_all_blks, post_all_blocks, mak_tab=TRUE)
```

##      Make phyloseq with true signal
```{r make_phyloseq}
all_real_taxa_lt <- unlist(all_taxa_res)
