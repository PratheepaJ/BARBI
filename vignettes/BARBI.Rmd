...
Analysis of shotgun-metagenomic sequencing samples that is a subset of plasma samples and all negative control samples from @cheng2017integration.
...
We identified three different blocks/batches of processing samples using a PCA on ranks of abundance (though we expect a user to know the blocks/batches in advance for each sample.) 
...
We make sure that we filter only the plasma, negative control, and healthy control samples.
...
We use the Bayesian inference to identify contaminants in each block because DNA contaminants are unique to each block. 
...
```{r list_of_phyloseq}
psBlockResult <- psBlockResults(ps, sampleTypeVar = "Sample_Type", caselevels = c("Healthy_Plasma","Patient_Plasma"), sampleName = "Sample_ID", blockVar = "block")
...
We estimate the gamma distribution parameters for the intensity of contaminants using the negative control samples for each block. 

`add1 == TRUE` corresponds to adding 1 to the otu table and `add1 == FALSE` corresponds to choosing 'poscounts' for size factor calculation method in `DESeq::estimateSizeFactors` and latter is the default option.
...
Choosing `stringent == TRUE` suggests to not using the scaling property for the species with the mean intensity of contamination less than one.
...
We can choose the option `Use_SNR` to tell whether the contamination identification is also based on the signal-to-noise ratio, otherwise, the contamination identification is based on the lower limit of the true signal and the upper limit of the contaminant. 
...
write.table(total_summary_table_all, 
            paste("./total_summary_table", ".txt", sep=""), 
            sep="\t", 
            col.names=TRUE, 
            row.names = FALSE)
