...
psPlByBlock <- lapply(psPlByBlock, function(x){
        subsetSample <- as.character(sample_data(x)$SampleCode[c(1:2,5:7)])
...
gammaPrior_all_blks <- lapply(blks, function(x){
        gammaPrior <- alphaBetaContInPlasma(
...
##     Make a table of the true signal for each samples.
...
for(blk in 1:num_blks){
        ...
        taxa.post.all.sam <- post_all_blocks[[blk]]
        gammPrior <- gammaPrior_all_blks[[blk]]
    
        total.summary.table <- NULL #Summary dataframe for all taxa from all subjects in a block.
        all.real.taxa <- character()

        
        ##########
        for(sam in 1:nsamples(psPlByBlock[[blk]])){
          
          taxa.post <- taxa.post.all.sam[[sam]]
          acceptance <- list()
          exp.post.s <- list()
          lower.s <- list()
          upper.s <- list()
          ...
          naive.exp <- list()
          all.zero.nc <- list()
                  for(taxa in 1:length(taxa.post)){
                          burnIn  <- burnIn
                          acceptance[[taxa]]  <-  1-mean(duplicated(taxa.post[[taxa]][-(1:burnIn),]))
                  
                          exp.post.s[[taxa]] <- mean(taxa.post[[taxa]][-(1:burnIn),])
                            
                          hdi.v <- hdi(taxa.post[[taxa]][-(1:burnIn),], credMass = cov.pro)
                          ...
                          sd.s[[taxa]] <- sd(taxa.post[[taxa]][-(1:burnIn),])
                  ...
                  mappingFileByBlock <-lapply(psByBlock, function(x) {as.data.frame(x@sam_data)})
                  ...
                  df <- data.frame(Species=taxa_names(psPlByBlock[[blk]]),
                                  ...
                                  Sample_Depth = rep(sampleDepth,length(unlist(upper.s))), Sample = rep(sample_names(psPlByBlock[[blk]])[sam],
                                                                                                        length(unlist(upper.s))))
  

                  #View(arrange(filter(df,(l.s>u.b)&(l.s>0)),desc(xj)))
                  #
                  ...
                  ps.tax.df.viruses <- filter(ps.tax.df, Superkingdom == "k__Viruses")
                  list.of.viruses <- as.character(ps.tax.df.viruses[,"Species"])
                  df <- arrange(filter(df, !(Species %in% list.of.viruses)), desc(xj))
                  ...
                  if(dim(df)[1]==0){
                          df <- data.frame(Species="Negative", xj="Negative", l.s="Negative", u.s="Negative", l.b ="Negative", u.b="Negative", sd.s = "Negative", naive.exp = "Negative", all.zero.nc = "Negative", Sample_Depth=sampleDepth, Sample = sample_names(psPlByBlock[[blk]])[sam])
                  }                  
                  
                  all.real.taxa <- c(all.real.taxa, as.character(df$Species))
...
#GGG
                  ...
                  for (i in 1:nrow(df)) {
                    ...
                    taxa.summary <- paste(df[i, "Species"], " (", df[i, "xj"], ", ", df[i, "l.s"], ", ", df[i, "u.b"], ", ", df[i, "all.zero.nc"], ")",  sep="")
                    sam.summary <- paste(sam.summary, "; ", taxa.summary, sep = "")
                    }
                  }
                  sam.summary <- substring(sam.summary, 3)
                  total.summary.table <- rbind(total.summary.table, data.frame(sample_names(psPlByBlock[[blk]])[sam], sam.summary))                 
                 ...
                 # Modify df columns for printing
                 df.mod <- df
                 df.mod <- subset(df.mod, select = c(Species, xj, l.s, u.b, all.zero.nc))
                 colnames(df.mod) <- c("Species", "Raw Reads", "Lower Limit Real","Upper Limit Contaminant", "In Neg. Cont.")
                 
                  df.p <- tableGrob(df.mod)
                  title <- textGrob(sample_names(psPlByBlock[[blk]])[sam], gp = gpar(fontsize = 14))
        ...
        all.real.taxa <- unique(all.real.taxa)
        final.results.list[[blk]] <- list(total.summary.table, all.real.taxa)
}
