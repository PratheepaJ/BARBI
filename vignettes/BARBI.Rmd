...
[Code](https://www.tutorialspoint.com/r/r_mean_median_mode.htm)
...
for(blk in 1:num_blks){
                ...
                for(sam in 1:nsamples(psPlByBlock[[blk]])){
                        ...
                        for(taxa in 1:length(taxa_post)){
                                xij <- as.numeric(gammPrior[[sam]][[3]][taxa])
                                burnIn  <- burnIn
                                ...
                                lambda_re[[taxa]] <- getmode(taxa_post[[taxa]][-(1:burnIn),])
                                pro_re[[taxa]] <- dpois(x = xij, lambda = lambda_re[[taxa]])
                                lambda_c[[taxa]] <- exp_bb
                                pro_c[[taxa]] <- dpois(x = xij, lambda_c[[taxa]])
                        ...
                        df <- data.frame(Species=taxa_names(psPlByBlock[[blk]]),
                                         ...
                                         lambda_re = unlist(lambda_re),
                                         lambda_c = unlist(lambda_c),
                                         pro_re = unlist(pro_re),
                                         pro_c = unlist(pro_c)
                                         )
