...
```{r summary_stat,message=FALSE,warning=FALSE}
...
We use the Bayesian inference to identify contaminants in each block/batch because DNA contaminants are unique to each batch. 

Thus, we split the phyloseq object corresponding to each block/batch and store as a list of phyloseq objects, `psByBlock`. 

We select negative control samples from each block/batch and store as a list of phyloseq objects, `psNCbyBlock`. 

We select all species that has prevalence of zero in all negative control samples for each block/batch and store as a list of phyloseq objects, `psallzeroInNC`.

We select all plasma samples from each block/batch and store as a list of phyloseq objects, `psPlByBlock`.
...
We estimate the gamma distribution parameters for the intensity of contaminants usning the negative control samples for each block/batch. 
...
We choose the block/batch (for example `block =3`) samples to identify DNA contaminants.

Then, for each sample in the selected block/batch, we estimate the gamma distribution parameters of intensity of DNA contamination using the scaling property of the gamma distribution.
...
##      Sampling from the posterior of the intensity of true signal

For all samples in the selected block/batch, for all taxa in each sample, we sampling from the posterior of the intensity of true signal using the Metropolis-Hasting MCMC. We can choose the number of MCMC using the option `itera`.

We save the gamma prior for the internsity of the contamination and the posterior samples.
...
##     Make a table of the true signal for each samples in the selected block/batch.
