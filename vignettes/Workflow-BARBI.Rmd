...
#devtools::load_all(".")
#sourceDirectory("../BARBI/R")
```

Read phyloseq 
```{r read-phylo,message=FALSE,warning=FALSE}
ps <- readRDS("/Users/jpratheepa31/Dropbox/GitHub/sepsis_project/SIRS/Data/ps_sirs_updated_apr_26_2018.rds")
```
...
saveRDS(knownContGamma_posteriorSignal,file = "./BARBI_Results/signalPosterior_gammaPrior.rds")
```


Save the results for each sample as a table in png format
```{r eval=FALSE}
knownContGamma_posteriorSignal <- readRDS(file="./BARBI_Results/signalPosterior_gammaPrior.rds")
...
for(sam in 1:nsamples(psPlByBlock[[blk]])){
        ...
        SNR <- list()
                ...
                #
                df <- dplyr::arrange(filter(df,(l.s>u.b)&(l.s>0)),desc(xj))
                if(dim(df)[1]==0){
                        df <- data.frame(Species=NA)
                }
                
                filname <- paste("./BARBI_Results/",sample_names(psPlByBlock[[blk]])[sam],".png",sep="")
